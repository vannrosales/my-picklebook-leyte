...
export default function PlayerSidebar({ isOpen, onClose }) {
    ...
    const isActive = (routePattern) => {
        if (routePattern === 'player.dashboard') return url === '/player-dashboard';
            ...
            `}>
                <div>
                    ...
                    <nav className="space-y-2">
                        ...
                        <Link 
                            ...
                            className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-all duration-200 ${
                                isActive('player.dashboard')
                                    ? 'bg-[#22C55E] text-white font-semibold shadow-sm hover:bg-[#1faff5] hover:scale-[1.02]'
                                    : 'font-medium text-[#71796F] hover:bg-white hover:text-gray-900 hover:shadow-xs hover:translate-x-1'
                            }`}
                        >
                        ...
                        {/* My Bookings */}
                        <Link 
                            href={route('player.bookings')} 
                            className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-all duration-200 ${
                                isActive('/player/bookings')
                                    ? 'bg-[#22C55E] text-white font-semibold shadow-sm hover:scale-[1.02]'
                                    : 'font-medium text-[#71796F] hover:bg-white hover:text-gray-900 hover:shadow-xs hover:translate-x-1'
                            }`}
                        >
                            <svg className="w-5 h-5 fill-none stroke-current" strokeWidth="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
